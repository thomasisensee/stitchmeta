...
* Python source files:
  * The Python package source files are located in the `src/stitchmeta` directory.
  * The file `src/stitchmeta.__main__.py` defines a command line interface that
    is accessible both as the command `stitchmeta` and via
    `python -m stitchmeta`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_stitchmeta.py` contains the unit tests for the package.
